...
";
    ...
    private async Task CheckGeofenceAsync()
    {
        ...
        // Calculate distance for all POIs and sort by nearest first
        var nearbyPois = _allPois
            .Select(poi =>
            {
                var lat = poi.Latitude != 0 ? poi.Latitude : GetDefaultLatitude(poi.Name ?? "");
                ...
                return new { Poi = poi, Distance = distance };
            })
            .Where(x => x.Distance <= triggerRadius)
            .OrderBy(x => x.Distance)
            .ToList();

        // Only play the nearest POI
        var nearest = nearbyPois.FirstOrDefault();
        if (nearest != null && !_visitedPoiCodes.Contains(nearest.Poi.Code ?? ""))
        {
            _visitedPoiCodes.Add(nearest.Poi.Code ?? "");
            await RecordGeofenceEventAsync(nearest.Poi.Id, "enter", nearest.Distance);
            await TriggerAutoPlayAsync(nearest.Poi.Code ?? "", nearest.Poi.Name ?? "");
        }
    }
